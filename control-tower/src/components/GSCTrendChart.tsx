...
function groupTrend(
  ...
  for (const r of rows) {
    ...
    const prev = m.get(bucket) || { impressions: 0, clicks: 0 };
...
export default function GSCTrendChart({
  ...
  const stats = useMemo(() => {
    const vals = points.map((p) => num(p[metric]));
    ...
    <div className="mapCard" style={{ padding: 14 }}>
      <div className="mapCardTop" style={{ alignItems: "center" }}>
        <div>
          <div className="mapCardTitle">
            {metric === "impressions" ? "Impressions" : "Clicks"} trend ({mode})
          </div>
      ...
      <div className="mapFrame" style={{ marginTop: 12, padding: 10 }}>
        ...
        <div className="mini" style={{ marginTop: 8, opacity: 0.75 }}>
          Estacionalidad: picos ayudan a detectar “expansiones Delta”, campañas
          o index bursts.
        </div>
      </div>
    </div>
